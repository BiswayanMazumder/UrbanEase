  ...
  return (
    <div className="svc-card">
      <div className="svc-banner">
        ...
        <img src={service.image} alt={service.title} className="svc-banner-img" />
      ...
      <div className="svc-details-row">
        <div className="svc-info">
          ...
          <p className="svc-starts">
            {service.startsAt ? `Starts at ₹${service.price?.toLocaleString()}` : `₹${service.price?.toLocaleString()}`}
          </p>
          {service.oldPrice && (
            <p style={{ fontSize: "12px", color: "#888", textDecoration: "line-through" }}>
              ₹{service.oldPrice.toLocaleString()}
            </p>
          )}
          {service.duration && (
            <p style={{ fontSize: "12px", color: "#545454" }}>• {service.duration}</p>
          )}
          {service.perUnit && (
            <p style={{ fontSize: "12px", color: "#07794C", marginTop: "4px" }}>{service.perUnit}</p>
          )}
        </div>
        <div className="svc-action-col">
...
const SECTIONS = [
  { id: "sec-3visit",    heading: "3-visit packs",      key: "three_visit_packs"   },
  { id: "sec-value",     heading: "Value deals",         key: "value_deals"         },
  { id: "sec-deepclean", heading: "One time deep clean", key: "one_time_deep_clean" },
  { id: "sec-mini",      heading: "Mini services",       key: "mini_services"       },
];
...
export default function BathroomCleaning() {
  ...
  // ── fetch bathroom cleaning data ──────────────────────────────────────────
  useEffect(() => {
    // 1. nav tabs
    fetch(`${BASE}/api/bathroom-cleaning/tabs`)
      .then((r) => r.json())
      .then((data) => {
        const t = data.data || [];
        setTabs(t);
    ...
    // 2. all services grouped by category (single request)
    fetch(`${BASE}/api/bathroom-cleaning/services`)
      .then((r) => r.json())
      .then((data) => setServicesMap(data.data || {}))
      .catch(console.error)
    ...
    <div className="wss-root">
      ...
      <div className="wss-body">
        ...
        <div className="wss-left">
          <h2 className="wss-title">Bathroom Cleaning</h2>
          <p className="wss-rating">⭐ 4.80 (5.6 M bookings)</p>
                ...
                {itemCount === 0 ? (
                  ...
                  <div className="cart-summary">
                    <div className="cart-summary-inner">
                      ...
                      <button className="view-cart-btn" onClick={()=>navigate("/checkout")}>
                        View Cart
                      </button>
                    </div>
                  </div>
